...
class MavenIntegrationTests {
	...
	void throwsExceptionWhenPlatformPublicationsGroupConfigure() {
		subject.getPlatformPublications().register("debug", it -> it.setGroupId("dev.example"));
		subject.getPlatformPublications().register("release", it -> it.setGroupId("dev.example"));
		assertThat(() -> subject.getPlatformPublications().finalizeNow(), throwsException(message("must not configure platform's group id")));
	}

	@Test
	void throwsExceptionWhenPlatformPublicationsVersionConfigure() {
		subject.getPlatformPublications().register("debug", it -> it.setVersion("4.2"));
		subject.getPlatformPublications().register("release", it -> it.setVersion("4.2"));
		assertThat(() -> subject.getPlatformPublications().finalizeNow(), throwsException(message("must not configure platform's version")));
	}
}
